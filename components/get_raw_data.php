...
function save_data($unix_begin,$data,$files_target_dir,$result_file_extension){
	...
	foreach($data as &$current_line){
		...
		if(is_array($ua_data)){
			$record['browser'] = $ua_data[0];
			$record['os'] = $ua_data[1];
		}
		else {
			$record['browser'] = DEFAULT_BROWSER;
			$record['os'] = DEFAULT_OS;
		}
...
if($fetched_user_agent_strings_count>0)
	alert('info','Fetched data about '.$fetched_user_agent_strings_count.' user agent strings from userstack.com');
elseif(VERBOSE)
	alert('secondary','Did not fetch any user agent strings from userstack.com');
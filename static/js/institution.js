$(function(){
	...
	const months_chart_object = create_chart(months_chart,"Sessions per month",[],[],{
		onClick: () => {//change the selected months when clicked on the graph

			const selected_month = months_chart_object.chart.getElementAtEvent(event)[0]._model.label;

			month_select.find('option').filter(function() {
	...
	if(chosen_year==='')//select last year if none selected
		year_select.find('option:last-child').attr('selected','selected');
	else {

		year_select.find('option').filter(function() {
